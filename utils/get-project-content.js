  ...
  return glob
    ...
    .filter(directory => {
      try {
        ...
        log(`✘ Invalid ${directory}/page-template.json, page template ignored`, {
          level: 'LOG_LEVEL_ERROR'
        });
    ...
    .map(
      ...
      directory => {
        ...
        return {
          ...
          metadata: {
            name: metadata.name,
            pageTemplateDefinitionPath: path.resolve(directory, 'page-definition.json')
          },
          definitionData: JSON.stringify(_readJSONSync(
            path.resolve(directory, 'page-definition.json')
          ))
        };
      }
    );
