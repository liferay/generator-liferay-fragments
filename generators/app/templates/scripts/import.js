...
const {askPortalData, getGroupId, getHost, getPassword, getUsername} = require('./portal');
...
_updateCollection = (collectionJSON, collectionKey, collectionDirectory) => {
	var answer = readline.question(`Collection "${collectionJSON.name}" already exists, update it including all the fragments? [A/y/n] `);
	...
	_getFragment(
		...
		(fragment) => {
			if (updateAll) {
				...
				var answer = readline.question(`Fragment "${name}" already exists, update it including all the fragments? [Y/n]`);
